...
@objc protocol StoriesScrollViewDelegate: AnyObject {
...
open class StoriesScrollView: UIScrollView, UIScrollViewDelegate {
    ...
    fileprivate var childViews = [StoriesProtocol]()
    ...
    open func addChildViews(_ views: [StoriesProtocol]) {
    ...
    open func addChildView(_ view: StoriesProtocol) {
        addChildViews([view])
    }
    ...
    fileprivate func transformViewsInScrollView(_ scrollView: UIScrollView) {
        ...
        if xOffset.truncatingRemainder(dividingBy: bounds.size.width) == 0 {
            let inde = Int(xOffset / bounds.size.width)
            childViews[inde].start()
        }
